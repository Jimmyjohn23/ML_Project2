...
% INSERTED
% ---------------------------------------------------------------------------
Of the 195 labelled lung recordings present in the HLS-CMDS release, a large
portion were exact duplicates of another file. Duplication was verified at the signal level as well as the byte
...
implemented with scikit-learn's \texttt{MLPClassifier} inside a
...
The \textbf{convolutional neural network} was implemented in PyTorch as
...
skorch \texttt{NeuralNetClassifier}, which exposes a scikit-learn
...
is a standard result in machine learning.
...
minority-class examples, while SMOTE creates new examples by
...
masking can create new examples that still look like realistic
...
well formed. The \texttt{imbalanced-learn} pipeline can combine
...
Of the 195 initial files, a large number were exact duplicates, leaving only 94
recordings. Had this step not been caught, accuracy would have looked deceptively high.
